<script>
    ...
    function transition() {
        ...
        interval = setInterval(() => {
            ...
            const titleSpan = document.getElementById("logo-container");
  ...
  if (!introPlayed)
  {
    ...
    onMount(() => {
        // Adjust this to match the duration of your animation
        setTimeout(() => {
        animationDone = true;
        // Emit an event or change a store value to notify the app the animation is done
        }, 5000); // Assume 5000ms = 5 seconds for the animation
    });
</script>
...
<style>
    ...
    .regular-text
    {
        ...
        display: flex;
        font-family: "DotGothic16", sans-serif;
        font-size: clamp(0.8rem, 3vw, 10rem);  
        transition: font-family 0.5s ease-in-out;
        color: #FFFEE9;
    }
...
{#if !animationDone}
    <div id="intro-wrapper" class="wrapper">
        <form id = "text-container" on:submit|preventDefault={handleSubmit}>
        <span id="arrow">&#8594</span>
